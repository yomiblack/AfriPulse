...
<template>
  <div class="relative group/card">
    <button
      ...
      class="absolute top-2 right-2 z-10 w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-all duration-150 focus:opacity-100"
      :class="[
        isSelected(country.cca2)
          ? 'opacity-100 bg-brand-500 text-white shadow-md'
          : 'opacity-100 sm:opacity-0 sm:group-hover/card:opacity-100 bg-white/90 dark:bg-gray-800/90 text-gray-500 dark:text-gray-400 hover:bg-brand-50 hover:text-brand-600 shadow-sm disabled:opacity-30 disabled:cursor-not-allowed',
      ]"
    >
      {{ isSelected(country.cca2) ? '✓' : '+' }}
    </button>
